...
ulong asus_get_fitimg_size(ulong flash_addr)
{
	ulong h_size, d_size;
	...
	if (fdt_magic(fdt_pt) == FDT_MAGIC)
		d_size = fdt_totalsize(fdt_pt);
	else
		d_size = 0;
//printf("asus_get_fitimg_size is %lu (%08x)\n",d_size,d_size);
	return d_size;
}
...
ulong genimg_get_image(ulong img_addr)
{
	...
	if (img_addr != (ram_addr+sizeof(image_header_t))) {
...
#define	THREE_SECONDS_BYTES	0x167A000
		...
		if ((d_size + h_size) > THREE_SECONDS_BYTES) {
			ra_flash_read((uchar*) ram_addr, img_addr, THREE_SECONDS_BYTES);
			asus_green_led_off();
			ra_flash_read((uchar*) ram_addr+THREE_SECONDS_BYTES, img_addr+THREE_SECONDS_BYTES, (d_size + h_size)-THREE_SECONDS_BYTES);
		} else {
			ra_flash_read((uchar*) ram_addr, img_addr, d_size + h_size);
			asus_green_led_off();
		}
#else
		ra_flash_read((uchar*) ram_addr, img_addr , d_size + h_size);
	...
	images.fit_total_len = d_size + h_size;
#endif	/* CONFIG_ASUS_PRODUCT */
...
#if 0 //defined(CONFIG_ASUS_PRODUCT)
		int i;
