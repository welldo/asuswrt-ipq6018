...
{
#if defined(CONFIG_ASUS_PRODUCT) && defined(CONFIG_DUAL_TRX)
#if 0 // TBD
	if (ret) { /* 1st image fail */
		char buf2[30];
		char *new_argv[2]={NULL, buf2};
		sprintf(buf2, "0x%X#config@1", CFG_KERN2_ADDR);
		bootm_start(cmdtp, flag, 2, new_argv);
		ret = bootm_find_os(cmdtp, flag, 2, new_argv);
		if (!ret) { /* 2nd image success */
			/* copy linux2 to linux1 */
			printf("Copy firmware from %x to %x, length %lx\n", CFG_KERN2_ADDR, CFG_KERN_ADDR, images->fit_total_len);
			ra_flash_erase_write((uchar *)images->os.start, CFG_KERN_ADDR, images->fit_total_len, 0);
		}
	} else {
		/* check linux2 & copy linux1 to linux2 if neccessary */
		ulong img_len;
		img_len=asus_get_fitimg_size(CFG_KERN2_ADDR);
		// printf("VVVVV img_len:%08lx, fit_total_len:%08lx\n", img_len, images->fit_total_len);
		if ((!img_len) || (img_len != images->fit_total_len)) {
			printf("Copy firmware from %x to %x, length %lx\n", CFG_KERN_ADDR, CFG_KERN2_ADDR, images->fit_total_len);
			ra_flash_erase_write((uchar *)images->os.start, CFG_KERN2_ADDR, images->fit_total_len, 0);
		}
	}
#endif
#endif
...
#if defined(PLAX56_XP4) || defined(RTAX5) || defined(RTMANGO) // rescue process indication
